...
import { useRejectBeras } from '../../lib/reject_beras';
...
const Kas = () => {
    const { data, error } = useRejectBeras()
    ...
    async function hapusTransaksi(id) {
        ...
        let res = await fetch(`/api/reject_beras/hapus?id=${id}`, { method: 'DELETE' })
        let json = await res.json()
        if (!res.ok) throw Error(json.message)
        mutate('/api/reject_beras/hapus')
        alert('Transaksi telah dihapus')
        Router.push('/reject_beras/reject_beras')
        ...
        <>
            ...
            <div className='mt-2'>
                <div className="jumbotron text-center" style={{ backgroundColor: '#25316D' }}>
                    <h3 className="display-12" style={{ color: 'orange' }}><em>Stok Reject Beras</em></h3>
                </div>
            </div>
...
function TransactionModal() {
    ...
    async function submitHandler(e) {
        ...
        try {
            const res = await fetch("http://localhost:3000/api/reject_beras/input", {
                method: 'POST',
                headers: {
        ...
        <>
            ...
            <Modal show={show} onHide={handleClose}>
                <Modal.Header closeButton>
                    <Modal.Title>Transaksi Reject Beras</Modal.Title>
                </Modal.Header>
                <Modal.Body>
...
function KasUpdate(props) {
    ...
    async function submitHandler(e) {
        ...
        try {
            const res = await fetch('/api/reject_beras/update', {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                },
