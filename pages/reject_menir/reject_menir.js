...
import { useRejectMenir } from '../../lib/reject_menir';
...
const Kas = () => {
    const { data, error } = useRejectMenir()
    ...
    async function hapusTransaksi(id) {
        ...
        let res = await fetch(`/api/reject_menir/hapus?id=${id}`, { method: 'DELETE' })
        let json = await res.json()
        if (!res.ok) throw Error(json.message)
        mutate('/api/reject_menir/hapus')
        alert('Transaksi telah dihapus')
        Router.push('/reject_menir/reject_menir')
        ...
        <>
            ...
            <div className='mt-2'>
                <div className="jumbotron text-center" style={{ backgroundColor: '#25316D' }}>
                    <h3 className="display-12" style={{ color: 'orange' }}><em>Stok Reject Menir</em></h3>
                </div>
            </div>
...
function TransactionModal() {
    ...
    async function submitHandler(e) {
        ...
        try {
            const res = await fetch("http://localhost:3000/api/reject_menir/input", {
                method: 'POST',
                headers: {
        ...
        <>
            ...
            <Modal show={show} onHide={handleClose}>
                <Modal.Header closeButton>
                    <Modal.Title>Transaksi Reject Menir</Modal.Title>
                </Modal.Header>
                <Modal.Body>
...
function KasUpdate(props) {
    ...
    async function submitHandler(e) {
        ...
        try {
            const res = await fetch('/api/reject_menir/update', {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                },
